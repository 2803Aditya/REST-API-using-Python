...
def add_data():
    ...
    if name and brand and price and request.method == 'POST':
        ...
        return jsonify('Object Added Successfully', 201)  # Success Code 201

    else:
        return jsonify('Operation Unsuccessful!!', 404)  # 404 Status Code object not found


'''
    This API Endpoint will delete an already existing object from our collection.
'''
@app.route('/api/Products/delete/<id>', methods = ['DELETE'])       # Allowed Method "DELETE"
def delete_obj(id):

    if request.method == 'DELETE':
        mongo.db.Titanic.delete_one({'_id' : ObjectId(id)})       #Querying the database
...
'''
    This API Endpoint will update an already existing object in our collection.
'''
@app.route('/api/Products/update/<id>', methods = ['PUT'])      # Allowed Method "PUT"
def update_obj(id):

    upd = request.json                  # Access the incoming data in Flask Application
...
'''
    This API Endpoint will find a new object in our collection.
'''
@app.route('/api/Products/find/<id>', methods = ["GET"])    # Allowed Method "GET"
def find_Obj(id):
